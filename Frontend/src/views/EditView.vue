...
const handleSubmit = async () => {
    try {
        await customerStore.editCustomer(customer, customerID);
    ...
    $('#customerForm').validate({
        rules: {
            ...
            email: {
                ...
                email: true,
                remote: {
                    url: `http://localhost/php_crud+vue/Backend/api/check_unique_api.php?id=${customerID}`, // Your server-side endpoint to check email uniqueness
                    type: 'post',
                    data: {
                        email: function () {
                            return $('#email').val();
            ...
            phone: {
                ...
                maxlength: 10,
                remote: {
                    url: `http://localhost/php_crud+vue/Backend/api/check_unique_api.php?id=${customerID}`, // Your server-side endpoint to check phone uniqueness
                    type: 'post',
                    data: {
                        phone: function () {
                            return $('#phone').val();
...
<template>
    <div class="container">
        <h1 class="mt-4 text-center">Edit Customer</h1>
